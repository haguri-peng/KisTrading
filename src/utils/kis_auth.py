...
def save_token(my_token, my_expired):
    ...
    with open(token_tmp, "w", encoding="utf-8") as f2:
        f2.write(f"token: {my_token}\n")
        f2.write(f"valid-date: {valid_date}\n")
...
def read_token():
    try:
        # 토큰이 저장된 파일 읽기
        with open(token_tmp, encoding="UTF-8") as f3:
            tkg_tmp = yaml.load(f3, Loader=yaml.FullLoader)
